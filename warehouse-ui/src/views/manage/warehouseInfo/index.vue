<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="warehouseInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="仓库位置" align="center" prop="location" v-if="columns[4].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="仓库面积" align="center" prop="area" v-if="columns[5].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="仓库管理员" align="center" prop="userName" v-if="columns[6].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="总容量" align="center" prop="totalCapacity" v-if="columns[7].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="当前数量" align="center" prop="currentQuantity" v-if="columns[8].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="入库总数" align="center" prop="inboundQuantity" v-if="columns[9].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="出库总数" align="center" prop="outboundQuantity" v-if="columns[10].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="联系电话" align="center" prop="contactPhone" v-if="columns[11].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="状态" align="center" prop="warehouseStatus" v-if="columns[12].visible">
        <template #default="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['manage:warehouseInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['manage:warehouseInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="warehouseInfoRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="仓库面积" prop="area">
          <el-input-number :min="0" style="width: 100%" :precision="2" v-model="form.area"
                           placeholder="请输入仓库面积"/>
        </el-form-item>
        ...
        <el-form-item label="总容量" prop="totalCapacity">
          <el-input-number :min="0" style="width: 100%" v-model="form.totalCapacity" placeholder="请输入总容量"/>
        </el-form-item>
...
import {
  ...
  addWarehouseInfo,
  updateWarehouseInfo
} from "@/api/manage/warehouseInfo";
import {allocatedUserListAll} from "@/api/system/role.js";

const {proxy} = getCurrentInstance();
const {
  warehouse_warehouse_type,
  warehouse_common_status
} = proxy.useDict('warehouse_warehouse_type', 'warehouse_common_status');
...
const data = reactive({
  ...
  columns: [
    ...
    {key: 8, label: '当前数量', visible: true},
    {key: 9, label: '入库总数', visible: true},
    {key: 10, label: '出库总数', visible: true},
    {key: 11, label: '联系电话', visible: true},
