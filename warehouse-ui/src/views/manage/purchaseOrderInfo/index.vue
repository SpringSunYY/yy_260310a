  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="purchaseOrderInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="编号" align="center" prop="orderId" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="采购订单号" align="center" prop="orderNo" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="供应商" align="center" prop="supplierId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="订单日期" align="center" prop="orderDate" width="180" v-if="columns[3].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.orderDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="预计到货日期" align="center" prop="expectedArrivalDate" width="180" v-if="columns[4].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.expectedArrivalDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="订单总金额" align="center" prop="totalAmount" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="申请人" align="center" prop="applicantId" v-if="columns[7].visible" :show-overflow-tooltip="true"/>
                ...
                <el-table-column label="审批人" align="center" prop="approverId" v-if="columns[9].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="审批时间" align="center" prop="approvalTime" width="180" v-if="columns[10].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.approvalTime, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="审批描述" align="center" prop="approvalDesc" v-if="columns[11].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:purchaseOrderInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:purchaseOrderInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="purchaseOrderInfoRef" :model="form" :rules="rules" label-width="80px">
                        <el-form-item label="采购订单号" prop="orderNo">
            ...
            <el-table :data="purchaseOrderDetailInfoList" :row-class-name="rowPurchaseOrderDetailInfoIndex" @selection-change="handlePurchaseOrderDetailInfoSelectionChange" ref="purchaseOrderDetailInfo">
              ...
              <el-table-column label="序号" align="center" prop="index" width="50"/>
                      ...
                      <el-table-column label="采购单价" prop="purchasePrice" width="150">
                        <template #default="scope">
                          <el-input v-model="scope.row.purchasePrice" placeholder="请输入采购单价" />
                        </template>
                      </el-table-column>
                      ...
                      <el-table-column label="已收货数量" prop="receivedQuantity" width="150">
                        <template #default="scope">
                          <el-input v-model="scope.row.receivedQuantity" placeholder="请输入已收货数量" />
                        </template>
                      </el-table-column>
            </el-table>
      </el-form>
...
<script setup name="PurchaseOrderInfo">
  ...
  const { proxy } = getCurrentInstance();
      const { warehouse_inbound_status, warehouse_applicant_status } = proxy.useDict('warehouse_inbound_status', 'warehouse_applicant_status');
