<template>
  <div class="app-container">
    ...
    >
                    <el-table-column label="仓库" prop="warehouseId" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                    <el-table-column label="库位编码" align="center" prop="locationCode" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                    <el-table-column label="库位名称" align="center" prop="locationName" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                    <el-table-column label="父级" align="center" prop="parentId" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:locationInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Plus" @click="handleAdd(scope.row)" v-hasPermi="['manage:locationInfo:add']">新增</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:locationInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="LocationInfo">
  import { listLocationInfo, getLocationInfo, delLocationInfo, addLocationInfo, updateLocationInfo } from "@/api/manage/locationInfo";

  const { proxy } = getCurrentInstance();
      const { warehouse_location_level, warehouse_common_status } = proxy.useDict('warehouse_location_level', 'warehouse_common_status');
