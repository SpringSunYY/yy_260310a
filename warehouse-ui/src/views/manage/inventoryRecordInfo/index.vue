    ...
    <el-table v-loading="loading" :data="inventoryRecordInfoList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
              <el-table-column label="记录ID" align="center" prop="id" v-if="columns[0].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="备件编号" align="center" prop="partsCode" v-if="columns[1].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="仓库" align="center" prop="warehouseId" v-if="columns[2].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="库位" align="center" prop="locationId" v-if="columns[3].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="库存数量" align="center" prop="quantity" v-if="columns[4].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="冻结数量" align="center" prop="frozenQuantity" v-if="columns[5].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="可用数量" align="center" prop="availableQuantity" v-if="columns[6].visible" :show-overflow-tooltip="true"/>
                <el-table-column label="有效期至" align="center" prop="expiryDate" width="180" v-if="columns[7].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.expiryDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="最后入库日期" align="center" prop="lastInboundDate" width="180" v-if="columns[8].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.lastInboundDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
                <el-table-column label="最后出库日期" align="center" prop="lastOutboundDate" width="180" v-if="columns[9].visible" :show-overflow-tooltip="true">
                <template #default="scope">
                  <span>{{ parseTime(scope.row.lastOutboundDate, '{y}-{m}-{d} {h}:{i}:{s}') }}</span>
                </template>
              </el-table-column>
        ...
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['manage:inventoryRecordInfo:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['manage:inventoryRecordInfo:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script setup name="InventoryRecordInfo">
  import { listInventoryRecordInfo, getInventoryRecordInfo, delInventoryRecordInfo, addInventoryRecordInfo, updateInventoryRecordInfo } from "@/api/manage/inventoryRecordInfo";
  ...
  const data = reactive({
    ...
    columns: [
              { key: 0, label: '记录ID', visible: true },
                ...
                { key: 5, label: '冻结数量', visible: true },
                { key: 6, label: '可用数量', visible: true },
                { key: 7, label: '有效期至', visible: true },
                { key: 8, label: '最后入库日期', visible: true },
                { key: 9, label: '最后出库日期', visible: true },
