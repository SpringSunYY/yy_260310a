  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="sparePartsInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="备件名称" align="center" prop="partsName" v-if="columns[2].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="规格型号" align="center" prop="specificationModel" v-if="columns[3].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="存储条件" align="center" prop="storageCondition" v-if="columns[7].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="单价" align="center" prop="unitPrice" v-if="columns[8].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="最小库存量" align="center" prop="minStock" v-if="columns[9].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="最大库存量" align="center" prop="maxStock" v-if="columns[10].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="当前库存量" align="center" prop="currentStock" v-if="columns[11].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="主供应商" align="center" prop="supplierName" v-if="columns[12].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['manage:sparePartsInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['manage:sparePartsInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="sparePartsInfoRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="备件编码" prop="partsCode">
        ...
        <el-form-item label="单价" prop="unitPrice">
          <el-input-number :min="0" style="width: 100%" :precision="2" v-model="form.unitPrice"
                           placeholder="请输入单价"/>
        </el-form-item>
        <el-form-item label="最小库存量" prop="minStock">
          <el-input-number :min="0" style="width: 100%" v-model="form.minStock" placeholder="请输入最小库存量"/>
        </el-form-item>
        <el-form-item label="最大库存量" prop="maxStock">
          <el-input-number :min="0" style="width: 100%" v-model="form.maxStock" placeholder="请输入最大库存量"/>
        </el-form-item>
...
import {
  ...
  delSparePartsInfo,
  addSparePartsInfo,
  updateSparePartsInfo
} from "@/api/manage/sparePartsInfo";
import {listSupplierInfo} from "@/api/manage/supplierInfo.js";

const {proxy} = getCurrentInstance();
const {
  warehouse_parts_category,
  warehouse_common_status
} = proxy.useDict('warehouse_parts_category', 'warehouse_common_status');
