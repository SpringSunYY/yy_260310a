<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="信用等级" prop="creditLevel">
        <el-select v-model="queryParams.creditLevel" style="width: 200px" placeholder="请选择信用等级" clearable>
          <el-option
              v-for="dict in warehouse_supplier_credit_level"
              :key="dict.value"
              :label="dict.label"
              :value="dict.value"
          />
    ...
    <el-table v-loading="loading" :data="supplierInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="供应商编码" align="center" prop="supplierCode" v-if="columns[1].visible"
      ...
      <el-table-column label="联系邮箱" align="center" prop="contactEmail" v-if="columns[5].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="地址" align="center" prop="address" v-if="columns[6].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="信用等级" align="center" prop="creditLevel" v-if="columns[7].visible">
        <template #default="scope">
          <dict-tag :options="warehouse_supplier_credit_level" :value="scope.row.creditLevel"/>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['manage:supplierInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['manage:supplierInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="supplierInfoRef" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="供应商编码" prop="supplierCode">
...
const {
  ...
  warehouse_supplier_credit_level
} = proxy.useDict('warehouse_common_status', 'warehouse_supplier_credit_level');
