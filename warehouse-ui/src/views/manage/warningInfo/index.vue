<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="warningInfoList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="备件编码" align="center" prop="partsCode" v-if="columns[1].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="备件名称" align="center" prop="partsName" v-if="columns[2].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="当前库存" align="center" prop="currentStock" v-if="columns[4].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="预警阈值" align="center" prop="thresholdStock" v-if="columns[5].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="差异数量" align="center" prop="differenceStock" v-if="columns[6].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="处理方式" align="center" prop="handlingMethod" v-if="columns[8].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="处理时间" align="center" prop="handingDate" width="180" v-if="columns[9].visible"
                       :show-overflow-tooltip="true">
        <template #default="scope">
          <span>{{ parseTime(scope.row.handingDate, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="处理人" align="center" prop="handingUserName" v-if="columns[10].visible"
                       :show-overflow-tooltip="true"/>
      <el-table-column label="处理描述" align="center" prop="handingRemark" v-if="columns[11].visible"
                       :show-overflow-tooltip="true"/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['manage:warningInfo:edit']">修改
          </el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['manage:warningInfo:remove']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="warningInfoRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="备件编码" prop="partsCode">
          <el-input :readonly="true" v-model="form.partsCode" placeholder="请输入备件编码"/>
        </el-form-item>
        <el-form-item label="备件名称" prop="partsName">
          <el-input :readonly="true" v-model="form.partsName" placeholder="请输入备件名称"/>
        </el-form-item>
        <el-form-item label="预警类型" prop="warnType">
          <el-select :disabled="true" v-model="form.warnType" placeholder="请选择预警类型">
            <el-option
                v-for="dict in warehouse_warning_type"
                :key="dict.value"
                :label="dict.label"
        ...
        <el-form-item label="当前库存" prop="currentStock">
          <el-input :readonly="true" v-model="form.currentStock" placeholder="请输入当前库存"/>
        </el-form-item>
        <el-form-item label="预警阈值" prop="thresholdStock">
          <el-input :readonly="true" v-model="form.thresholdStock" placeholder="请输入预警阈值"/>
        </el-form-item>
        <el-form-item label="差异数量" prop="differenceStock">
          <el-input :readonly="true" v-model="form.differenceStock" placeholder="请输入差异数量"/>
        </el-form-item>
        <el-form-item label="预警状态" prop="warningStatus">
...
import {
  ...
  addWarningInfo,
  updateWarningInfo
} from "@/api/manage/warningInfo";
import {allocatedUserListAll} from "@/api/system/role.js";

const {proxy} = getCurrentInstance();
const {
  warehouse_warning_status,
  warehouse_warning_type
} = proxy.useDict('warehouse_warning_status', 'warehouse_warning_type');
